...
function Home() {
  ...
  const movies = moviesData.map((data, i) => {
    return <Movie key={i} title={data.title} overview={data.overview} poster={data.poster} voteAverage={data.voteAverage} voteCount={data.voteCount} updateLikedMovies={updateLikedMovies}/>;
  });

  const titrecolonne = likedMovies.map((dati, i) => {
    return (
      <div className={styles.deletetitle}>
        <span key={i}>{dati}</span>
        <button className={styles.delete} onClick={() => updateLikedMovies(dati)} >X</button>
      </div> )
  })
